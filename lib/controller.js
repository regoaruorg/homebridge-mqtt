...
Controller.prototype.addAccessory = function(api_accessory) {
  ...
  if (ack) {
    ...
    this.setAccessoryInformation({"name":name,"manufacturer":plugin_name,"model": plugin_v,"serialnumber":now,"group_name":"custom"}, false);
  }
...
Controller.prototype.setAccessoryInformation = function(accessory) {
  ...
  if (typeof this.hap_accessories[name] === "undefined") {
    ...
    service.setCharacteristic("group_name", "custom");
    if (ack) {
...
Controller.prototype.identify = function (name) {
  ...
  this.log("identify name '%s' manufacturer '%s' model '%s' serialnumber '%s' group_name 'custom'", name, manufacturer, model, serialnumber);
    
  this.Model.identify(name, manufacturer, model, serialnumber, "custom");
}
